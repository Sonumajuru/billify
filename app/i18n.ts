export type Lang = "en" | "fr" | "nl";

export const T = {
  en: {
    // App shell
    invoiceMode: "Invoice",
    tenancyMode: "Rent Receipt",
    generalReceiptMode: "General Receipt",
    invoiceModeDesc: "Invoices, quotes & credit notes",
    tenancyModeDesc: "Rental receipts for landlords",
    generalReceiptModeDesc: "Receipts for goods & services",
    hidePreview: "Hide Preview",
    showPreview: "Show Preview",
    downloadPdf: "Download PDF",
    generating: "Generating…",
    autosaved: "Autosaved",
    reset: "Reset",
    resetConfirm: "Reset all fields?",
    content: "Content",
    templates: "Templates",
    saved: "Saved",
    preview: "Preview",
    form: "Form",

    // Common
    advanced: "Advanced options",
    hideAdvanced: "Hide advanced",
    logo: "Logo",
    uploadLogo: "Upload logo",
    uploadLogoSub: "PNG, JPG, SVG",
    fullName: "Full Name",
    email: "Email",
    phone: "Phone",
    company: "Company",
    address: "Address",
    postcode: "Postcode",
    city: "City",
    country: "Country",
    currency: "Currency",
    issueDate: "Issue Date",
    status: "Status",
    method: "Method",
    signatureLines: "Include signature lines",
    watermark: "Show watermark",

    // Invoice sections
    yourInfo: "Your Info",
    documentSection: "Document",
    vatSchemeSection: "VAT Scheme",
    clientSection: "Client",
    projectSection: "Project",
    itemsSection: "Line Items",
    paymentSection: "Payment",

    // Invoice — Your Info
    jobTitle: "Job Title / Role",
    website: "Website",
    kvk: "KvK Number",
    btw: "VAT Number (BTW)",
    iban: "IBAN",
    bic: "BIC / SWIFT",
    bankName: "Bank Name",
    dutchBusiness: "Dutch Business Details",
    bankDetails: "Bank Details",

    // Invoice — Document
    docType: "Document Type",
    invoiceNumber: "Invoice Number",
    dueDate: "Due Date",
    paymentTerms: "Payment Terms",
    poNumber: "PO Number",
    clientReference: "Client Reference",
    days: "days",

    // Invoice — VAT
    vatStandard: "Standard VAT",
    vatStandardDesc: "21% / 9% / 0% — set per line item",
    vatReverse: "Reverse Charge (Verlegd)",
    vatReverseDesc: "Invoice without VAT — client pays (art. 12 OB)",
    vatExempt: "VAT Exempt",
    vatExemptDesc: "Freelancer under €20,000/yr or KOR",
    vatEu: "EU / Intra-Community",
    vatEuDesc: "EU client with VAT number — 0% reverse charge",

    // Invoice — Client
    contactName: "Contact Name",
    clientCompany: "Company Name",
    clientKvk: "Client KvK",
    clientVat: "Client VAT Number",

    // Invoice — Project
    projectName: "Project Name",
    techStack: "Technology / Platform",
    periodFrom: "Period From",
    periodTo: "Period To",
    projectDesc: "Project Description",

    // Invoice — Items
    addItem: "Add line item",
    qty: "Qty",
    unit: "Unit",
    rate: "Rate",
    vatPct: "VAT %",

    // Invoice — Payment
    paymentMethod: "Payment Method",
    paymentInstructions: "Payment Instructions",
    discountAmount: "Discount",
    discountType: "Type",
    discountPct: "Percentage (%)",
    discountFixed: "Fixed amount",

    // Tenancy sections
    landlordSection: "Landlord / Manager",
    receiptSection: "Receipt Details",
    tenantSection: "Tenant",
    propertySection: "Property",
    chargesSection: "Charges",

    // Tenancy — Landlord
    landlordName: "Your Name",
    landlordCompany: "Company / Agency",
    landlordAddress: "Address",

    // Tenancy — Receipt
    receiptNumber: "Receipt #",
    paymentDate: "Payment Date",

    // Tenancy — Tenant
    tenantName: "Tenant Name",

    // Tenancy — Property
    propertyAddress: "Property Address",
    propertyUnit: "Unit / Apt #",
    propertyType: "Type",

    // Tenancy — Charges
    monthlyRent: "Monthly Rent",
    moreCharges: "Optional charges",
    hideCharges: "Hide optional charges",
    lateFee: "Late Fee",
    parkingFee: "Parking Fee",
    utilities: "Utilities",
    maintenance: "Maintenance",
    deposit: "Security Deposit",
    otherFee: "Other",
    discount: "Discount",

    // Doc types
    docInvoice: "Invoice",
    docQuote: "Quote / Estimate",
    docProforma: "Pro Forma Invoice",
    docCredit: "Credit Note",
    docReceipt: "Receipt",

    // Preview / PDF — Invoice
    pDocInvoice:"INVOICE", pDocQuote:"QUOTE", pDocProforma:"PRO FORMA INVOICE", pDocCredit:"CREDIT NOTE", pDocReceipt:"RECEIPT",
    pFrom:"From", pBillTo:"Bill To", pTo:"To",
    pPaymentDetails:"Payment Details", pAccName:"Acc. name:", pMethod:"Method:", pDueDate:"Due date:", pTerms:"Terms:", pRef:"Ref.:",
    pSubtotal:"Subtotal (excl. VAT)", pDiscount:"Discount", pTaxGroup:"VAT", pTotal:"Total (incl. VAT)",
    pVatReversed:"VAT reverse charged – art. 12 OB Act 1968", pVatExempt:"Exempt from VAT",
    pIntraCommunity:"Intra-community supply – VAT reverse charged to",
    pSignatureProvider:"Service Provider", pSignatureClient:"Client",
    pProject:"Project", pTech:"Technology", pPeriod:"Period",
    pCodeFrom:"from", pCodeBillTo:"bill_to", pCodeProject:"project",

    // Preview / PDF — Tenancy
    pTenancyReceipt:"Tenancy Receipt", pRentReceipt:"Rent Receipt", pOfficialReceipt:"Official Receipt",
    pReceivedFrom:"Received From", pForProperty:"For Property", pTenantDetails:"Tenant Details", pPropertyDetails:"Property Details",
    pTenantLabel:"Tenant", pPropertyLabel:"Property",
    pMonthlyRent:"Monthly Rent", pTotalPaid:"Total Amount Paid", pTotalPaidShort:"Total Paid", pTotalPaidFormal:"TOTAL PAID",
    pPaymentMethodLabel:"Payment Method:", pPaidOn:"Paid On:", pPaidVia:"Via:", pOn:"On:",
    pLandlord:"Landlord", pTenant:"Tenant", pSignatureSuffix:"Signature",
    pTermsLabel:"Terms:", pIssuedLabel:"Issued:", pDateIssued:"Date Issued:", pPaymentDateLabel:"Payment Date:",
    pLateFee:"Late Fee", pParkingFee:"Parking Fee", pSecurityDeposit:"Security Deposit",

    // General Receipt
    grSellerSection: "Your Info",
    grBuyerSection: "Customer",
    grReceiptSection: "Receipt Details",
    grItemsSection: "Items",
    grBuyerName: "Customer Name",
    grItemDesc: "Description",
    grQty: "Qty",
    grPrice: "Price",
    grTaxLabel: "Tax Label",
    grTaxRate: "Tax Rate (%)",
    grShowTax: "Include tax line",
    pGrSoldTo: "Sold To",
    pGrSubtotal: "Subtotal",
    pGrTotal: "Total",
    pGrTax: "Tax",
    pGrThankYou: "Thank you for your business!",
  },

  fr: {
    invoiceMode: "Facture",
    tenancyMode: "Loyer",
    generalReceiptMode: "Reçu Général",
    invoiceModeDesc: "Factures, devis & avoirs",
    tenancyModeDesc: "Quittances de loyer",
    generalReceiptModeDesc: "Reçus pour biens & services",
    hidePreview: "Masquer l'aperçu",
    showPreview: "Afficher l'aperçu",
    downloadPdf: "Télécharger PDF",
    generating: "Génération…",
    autosaved: "Sauvegardé",
    reset: "Réinitialiser",
    resetConfirm: "Réinitialiser tous les champs ?",
    content: "Contenu",
    templates: "Modèles",
    saved: "Sauvegardés",
    preview: "Aperçu",
    form: "Formulaire",

    advanced: "Options avancées",
    hideAdvanced: "Masquer avancé",
    logo: "Logo",
    uploadLogo: "Télécharger logo",
    uploadLogoSub: "PNG, JPG, SVG",
    fullName: "Nom complet",
    email: "Email",
    phone: "Téléphone",
    company: "Société",
    address: "Adresse",
    postcode: "Code postal",
    city: "Ville",
    country: "Pays",
    currency: "Devise",
    issueDate: "Date d'émission",
    status: "Statut",
    method: "Mode",
    signatureLines: "Lignes de signature",
    watermark: "Afficher filigrane",

    yourInfo: "Mes Informations",
    documentSection: "Document",
    vatSchemeSection: "Régime TVA",
    clientSection: "Client",
    projectSection: "Projet",
    itemsSection: "Lignes",
    paymentSection: "Paiement",

    jobTitle: "Titre / Fonction",
    website: "Site web",
    kvk: "N° KvK",
    btw: "N° TVA (BTW)",
    iban: "IBAN",
    bic: "BIC / SWIFT",
    bankName: "Banque",
    dutchBusiness: "Détails entreprise NL",
    bankDetails: "Coordonnées bancaires",

    docType: "Type de document",
    invoiceNumber: "N° Facture",
    dueDate: "Échéance",
    paymentTerms: "Délai de paiement",
    poNumber: "N° Commande",
    clientReference: "Référence client",
    days: "jours",

    vatStandard: "TVA standard",
    vatStandardDesc: "21% / 9% / 0% — par ligne",
    vatReverse: "Autoliquidation (Verlegd)",
    vatReverseDesc: "Sans TVA — client redevable (art. 12 OB)",
    vatExempt: "Exonération TVA",
    vatExemptDesc: "Micro-entrepreneur ou service exonéré",
    vatEu: "UE / Intracommunautaire",
    vatEuDesc: "Client UE avec n° TVA — 0% autoliquidation",

    contactName: "Nom du contact",
    clientCompany: "Société client",
    clientKvk: "KvK client",
    clientVat: "N° TVA client",

    projectName: "Nom du projet",
    techStack: "Technologies",
    periodFrom: "Période du",
    periodTo: "Au",
    projectDesc: "Description",

    addItem: "Ajouter une ligne",
    qty: "Qté",
    unit: "Unité",
    rate: "Prix",
    vatPct: "TVA %",

    paymentMethod: "Mode de paiement",
    paymentInstructions: "Instructions",
    discountAmount: "Remise",
    discountType: "Type",
    discountPct: "Pourcentage (%)",
    discountFixed: "Montant fixe",

    landlordSection: "Propriétaire / Gérant",
    receiptSection: "Quittance",
    tenantSection: "Locataire",
    propertySection: "Bien immobilier",
    chargesSection: "Charges",

    landlordName: "Votre nom",
    landlordCompany: "Société / Agence",
    landlordAddress: "Adresse",

    receiptNumber: "N° Quittance",
    paymentDate: "Date de paiement",

    tenantName: "Nom du locataire",

    propertyAddress: "Adresse du bien",
    propertyUnit: "Apt / Unité",
    propertyType: "Type",

    monthlyRent: "Loyer mensuel",
    moreCharges: "Charges optionnelles",
    hideCharges: "Masquer les charges",
    lateFee: "Majoration retard",
    parkingFee: "Parking",
    utilities: "Charges (eau/elec.)",
    maintenance: "Entretien",
    deposit: "Dépôt de garantie",
    otherFee: "Autre",
    discount: "Remise",

    docInvoice: "Facture",
    docQuote: "Devis",
    docProforma: "Facture proforma",
    docCredit: "Avoir",
    docReceipt: "Reçu",

    pDocInvoice:"FACTURE", pDocQuote:"DEVIS", pDocProforma:"FACTURE PROFORMA", pDocCredit:"AVOIR", pDocReceipt:"REÇU",
    pFrom:"De", pBillTo:"Facturer à", pTo:"À",
    pPaymentDetails:"Détails de paiement", pAccName:"Titulaire:", pMethod:"Mode:", pDueDate:"Échéance:", pTerms:"Conditions:", pRef:"Réf.:",
    pSubtotal:"Sous-total (HT)", pDiscount:"Remise", pTaxGroup:"TVA", pTotal:"Total (TTC)",
    pVatReversed:"TVA autoliquidée – art. 12 OB", pVatExempt:"Exonéré de TVA",
    pIntraCommunity:"Livraison intracommunautaire – TVA autoliquidée à",
    pSignatureProvider:"Prestataire", pSignatureClient:"Client",
    pProject:"Projet", pTech:"Technologie", pPeriod:"Période",
    pCodeFrom:"de", pCodeBillTo:"facturer_à", pCodeProject:"projet",

    pTenancyReceipt:"Quittance de loyer", pRentReceipt:"Reçu de loyer", pOfficialReceipt:"Reçu officiel",
    pReceivedFrom:"Reçu de", pForProperty:"Pour le bien", pTenantDetails:"Infos locataire", pPropertyDetails:"Infos bien",
    pTenantLabel:"Locataire", pPropertyLabel:"Bien",
    pMonthlyRent:"Loyer mensuel", pTotalPaid:"Total versé", pTotalPaidShort:"Total payé", pTotalPaidFormal:"TOTAL PAYÉ",
    pPaymentMethodLabel:"Mode de paiement:", pPaidOn:"Payé le:", pPaidVia:"Via:", pOn:"Le:",
    pLandlord:"Propriétaire", pTenant:"Locataire", pSignatureSuffix:"Signature",
    pTermsLabel:"Conditions:", pIssuedLabel:"Émis le:", pDateIssued:"Date d'émission:", pPaymentDateLabel:"Date de paiement:",
    pLateFee:"Majoration retard", pParkingFee:"Parking", pSecurityDeposit:"Dépôt de garantie",

    // General Receipt
    grSellerSection: "Vos Informations",
    grBuyerSection: "Client",
    grReceiptSection: "Détails du reçu",
    grItemsSection: "Articles",
    grBuyerName: "Nom du client",
    grItemDesc: "Description",
    grQty: "Qté",
    grPrice: "Prix",
    grTaxLabel: "Libellé taxe",
    grTaxRate: "Taux taxe (%)",
    grShowTax: "Inclure ligne taxe",
    pGrSoldTo: "Vendu à",
    pGrSubtotal: "Sous-total",
    pGrTotal: "Total",
    pGrTax: "Taxe",
    pGrThankYou: "Merci de votre confiance!",
  },

  nl: {
    invoiceMode: "Factuur",
    tenancyMode: "Huurkwitantie",
    generalReceiptMode: "Algemeen Bonnetje",
    invoiceModeDesc: "Facturen, offertes & nota's",
    tenancyModeDesc: "Huurkwitanties voor verhuurders",
    generalReceiptModeDesc: "Bonnetjes voor goederen & diensten",
    hidePreview: "Voorbeeld verbergen",
    showPreview: "Voorbeeld tonen",
    downloadPdf: "PDF downloaden",
    generating: "Genereren…",
    autosaved: "Opgeslagen",
    reset: "Reset",
    resetConfirm: "Alle velden wissen?",
    content: "Inhoud",
    templates: "Ontwerp",
    saved: "Opgeslagen",
    preview: "Voorbeeld",
    form: "Formulier",

    advanced: "Geavanceerde opties",
    hideAdvanced: "Verbergen",
    logo: "Logo",
    uploadLogo: "Logo uploaden",
    uploadLogoSub: "PNG, JPG, SVG",
    fullName: "Volledige naam",
    email: "E-mail",
    phone: "Telefoon",
    company: "Bedrijfsnaam",
    address: "Adres",
    postcode: "Postcode",
    city: "Stad",
    country: "Land",
    currency: "Valuta",
    issueDate: "Factuurdatum",
    status: "Status",
    method: "Methode",
    signatureLines: "Handtekeningregels",
    watermark: "Watermerk tonen",

    yourInfo: "Mijn Gegevens",
    documentSection: "Document",
    vatSchemeSection: "BTW-regeling",
    clientSection: "Klant",
    projectSection: "Project",
    itemsSection: "Regelitems",
    paymentSection: "Betaling",

    jobTitle: "Functietitel",
    website: "Website",
    kvk: "KvK-nummer",
    btw: "BTW-nummer",
    iban: "IBAN",
    bic: "BIC / SWIFT",
    bankName: "Banknaam",
    dutchBusiness: "Nederlandse bedrijfsgegevens",
    bankDetails: "Bankgegevens",

    docType: "Documenttype",
    invoiceNumber: "Factuurnummer",
    dueDate: "Vervaldatum",
    paymentTerms: "Betalingstermijn",
    poNumber: "Bestelnummer",
    clientReference: "Klantreferentie",
    days: "dagen",

    vatStandard: "Standaard BTW",
    vatStandardDesc: "21% / 9% / 0% — per regelitem",
    vatReverse: "BTW Verlegd",
    vatReverseDesc: "Zonder BTW — klant betaalt (art. 12 OB)",
    vatExempt: "BTW Vrijgesteld",
    vatExemptDesc: "Freelancer onder €20.000/jr of KOR",
    vatEu: "EU / Intracommunautair",
    vatEuDesc: "EU-klant met BTW-nummer — 0% verlegd",

    contactName: "Contactpersoon",
    clientCompany: "Bedrijfsnaam klant",
    clientKvk: "KvK klant",
    clientVat: "BTW-nummer klant",

    projectName: "Projectnaam",
    techStack: "Technologie / Platform",
    periodFrom: "Periode van",
    periodTo: "Tot",
    projectDesc: "Projectomschrijving",

    addItem: "Regel toevoegen",
    qty: "Aantal",
    unit: "Eenheid",
    rate: "Tarief",
    vatPct: "BTW %",

    paymentMethod: "Betaalmethode",
    paymentInstructions: "Betalingsinstructies",
    discountAmount: "Korting",
    discountType: "Type",
    discountPct: "Percentage (%)",
    discountFixed: "Vast bedrag",

    landlordSection: "Verhuurder / Beheerder",
    receiptSection: "Kwitantiegegevens",
    tenantSection: "Huurder",
    propertySection: "Woning",
    chargesSection: "Betalingsoverzicht",

    landlordName: "Uw naam",
    landlordCompany: "Bedrijf / Makelaar",
    landlordAddress: "Adres",

    receiptNumber: "Kwitantienummer",
    paymentDate: "Betaaldatum",

    tenantName: "Naam huurder",

    propertyAddress: "Adres woning",
    propertyUnit: "Appartement / Nr.",
    propertyType: "Type",

    monthlyRent: "Maandhuur",
    moreCharges: "Optionele kosten",
    hideCharges: "Kosten verbergen",
    lateFee: "Boete te laat",
    parkingFee: "Parkeerkosten",
    utilities: "Nutsvoorzieningen",
    maintenance: "Onderhoud",
    deposit: "Borg",
    otherFee: "Overig",
    discount: "Korting",

    docInvoice: "Factuur",
    docQuote: "Offerte / Schatting",
    docProforma: "Pro-formafactuur",
    docCredit: "Creditnota",
    docReceipt: "Kwitantie",

    pDocInvoice:"FACTUUR", pDocQuote:"OFFERTE", pDocProforma:"PROFORMAFACTUUR", pDocCredit:"CREDITNOTA", pDocReceipt:"KWITANTIE",
    pFrom:"Van", pBillTo:"Factuur aan", pTo:"Aan",
    pPaymentDetails:"Betalingsgegevens", pAccName:"Rekeninghouder:", pMethod:"Methode:", pDueDate:"Vervaldatum:", pTerms:"Voorwaarden:", pRef:"Ref.:",
    pSubtotal:"Subtotaal (excl. BTW)", pDiscount:"Korting", pTaxGroup:"BTW", pTotal:"Totaal (incl. BTW)",
    pVatReversed:"BTW verlegd – art. 12 OB 1968", pVatExempt:"Vrijgesteld van BTW",
    pIntraCommunity:"Intracommunautaire levering – BTW verlegd naar",
    pSignatureProvider:"Dienstverlener", pSignatureClient:"Klant",
    pProject:"Project", pTech:"Technologie", pPeriod:"Periode",
    pCodeFrom:"van", pCodeBillTo:"factuur_aan", pCodeProject:"project",

    pTenancyReceipt:"Huurkwitantie", pRentReceipt:"Huurontvangst", pOfficialReceipt:"Officieel bewijs",
    pReceivedFrom:"Ontvangen van", pForProperty:"Voor woning", pTenantDetails:"Huurdergegevens", pPropertyDetails:"Woninggegevens",
    pTenantLabel:"Huurder", pPropertyLabel:"Woning",
    pMonthlyRent:"Maandhuur", pTotalPaid:"Totaal betaald", pTotalPaidShort:"Totaal betaald", pTotalPaidFormal:"TOTAAL BETAALD",
    pPaymentMethodLabel:"Betaalmethode:", pPaidOn:"Betaald op:", pPaidVia:"Via:", pOn:"Op:",
    pLandlord:"Verhuurder", pTenant:"Huurder", pSignatureSuffix:"Handtekening",
    pTermsLabel:"Voorwaarden:", pIssuedLabel:"Uitgegeven:", pDateIssued:"Uitgieftedatum:", pPaymentDateLabel:"Betaaldatum:",
    pLateFee:"Boete te laat", pParkingFee:"Parkeerkosten", pSecurityDeposit:"Borg",

    // General Receipt
    grSellerSection: "Uw Gegevens",
    grBuyerSection: "Klant",
    grReceiptSection: "Kwitantiegegevens",
    grItemsSection: "Artikelen",
    grBuyerName: "Naam klant",
    grItemDesc: "Omschrijving",
    grQty: "Aantal",
    grPrice: "Prijs",
    grTaxLabel: "Belastingsoort",
    grTaxRate: "Belasting (%)",
    grShowTax: "Belastingregel tonen",
    pGrSoldTo: "Verkocht aan",
    pGrSubtotal: "Subtotaal",
    pGrTotal: "Totaal",
    pGrTax: "Belasting",
    pGrThankYou: "Bedankt voor uw aankoop!",
  },
} as const;

export type Tr = { [K in keyof typeof T.en]: string };
